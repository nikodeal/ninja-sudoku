...
function generateRandomMatrix() { //generate ramdom matrix from given 12 matrixes;
    ...
    randomNum = Math.floor((Math.random() * 12)); // got a randomized number // מעגל את המספרים כלפי מטה ונותן מספרים רנדומלים
...
const logIn = (user, pass) => {
    ...
    user = document.getElementById("username-input").value;  //we grab the username input יוזר שווה לשם המשתמש
    pass = document.getElementById("password-input").value;  ///then we grab the password input pass = שווה לסיסמא
    document.getElementById('welcomePageTwoSpan').innerHTML = user; // this is the welcome page in

    if (user === 'test' && pass === 'test') { // check for match בדיקת התאמה בין שם המשתמש והסיסמא
        // if there is a match 
        pageDisplay("none", "block", "none");
    } else {
        errorPageOne.innerHTML = "The values provided do not exist in our database" //else we put a message that there is no match // אם אין התאמה בין השם משתמש והסיסמא אז תהיה הודעה שאומרת את זה
    }
};
...
const addListenersToEmptyCells = () => {  //here we have a function to add event listeners to the empty cells
    for (i = 0; i < allCells.length; i++) { // we loop through all the cells
        if(allCells[i].textContent !== ''){
            allCells[i].readOnly = true;
        }
       else if (allCells[i].textContent === "") { //if we find an empty cell
